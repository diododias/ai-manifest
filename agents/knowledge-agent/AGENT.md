...
- priorize `/update-docs`, `/refine-spec` e `/technical-discovery`. Declare no envelope os nomes exatos em `skills_used`; se nenhuma skill de dominio se aplicar, registre o motivo.
...
- **Conhecimento:** fonte canônica do domínio; decisões técnicas em `engineering/adr/` quando aplicável.
- **Aprendizados e review:** `<tech-lead-workspace>/projects/<project>/LEARNINGS.md` e `execution/reviews/knowledge-<id>.md`; propostas não aprovadas em `.coordination/`.
