...
- priorize `/technical-discovery`, `/analyse-bug` e `/update-docs`. Declare no envelope os nomes exatos em `skills_used`; se nenhuma skill de dominio se aplicar, registre o motivo.
...
- **Dataset, qualidade e relatório:** `<tech-lead-workspace>/projects/<project>/execution/telemetry/<periodo>.md`.
- Preserve esquema, origem, cobertura, retenção e limitações; não persista dados sensíveis.
