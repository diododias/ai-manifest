...
**dora-cli** complements Serena with a layer of observability over the development process itself — not the codebase, but how it was produced. It calculates delivery signals from git and GitHub history: deployment frequency, change lead time, change fail rate, failed deployment recovery time and deployment rework rate. In repositories with multiple agent sessions operating in parallel, this is the signal that shows whether the pace of change is actually improving delivery, not just producing more commits. The balanced interpretation, including raw deployment and failure counts, is in [Metrics](METRICS.md#delivery-and-stability).
