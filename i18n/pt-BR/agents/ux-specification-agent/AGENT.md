...
- priorize `/business-discovery`, `/write-feature` e `/update-docs` para artefatos de experiencia. Declare no envelope os nomes exatos em `skills_used`; se nenhuma skill de dominio se aplicar, registre o motivo.
...
- **Pesquisa e jornada:** `<ux-workspace>/projects/<project>/research/` e `journeys/`.
- **UX:** `flows/`, `specifications/<UXSPEC-id>.md`, `validation/<validation-id>.md` e `prototypes/` quando proporcional ao risco.
