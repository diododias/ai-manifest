...
- priorize `/review-spec`, `/review-cross-prd-spec` e `/technical-discovery`. Declare no envelope os nomes exatos em `skills_used`; se nenhuma skill de dominio se aplicar, registre o motivo.
...
- Não edite `PLAN`, `SPEC` ou `ADR` do autor; cada finding inclui evidência, cenário, impacto e ação.
