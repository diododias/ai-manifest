...
- priorize `/test-integration-local`, `/analyse-bug` e `/update-docs`. Declare no envelope os nomes exatos em `skills_used`; se nenhuma skill de dominio se aplicar, registre o motivo.
...
- **Matriz e falhas reproduzíveis:** `<tech-lead-workspace>/projects/<project>/execution/evidence/<WI-id>.md`.
- **Estado:** `work-items/<WI-id>.md` e `STATUS.md`, vinculando ambiente e cenário à evidência.
